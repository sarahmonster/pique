...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function pique_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'pique' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'pique_wp_title', 10, 2 );
	 ...
	 */
	function pique_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'pique_render_title' );
endif;
